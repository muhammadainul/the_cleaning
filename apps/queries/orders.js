...
exports.findByCustomerIdDate = ({ customerId, today }) =>
    new Promise(async (resolve, reject) => {
        try {
            let response = await conn.query(`select tbl_orders.id, tbl_orders.orderDate, tbl_pricelistjob.jobType, tbl_orders.orderStatus from tbl_orders 
                inner join tbl_priceListJob on tbl_orders.priceListId=tbl_priceListJob.id where customerId='${customerId}' and orderDate='${today}'`, (err, result) => {
                if (err) throw err
...
exports.findById = ({ id, date }) => 
    new Promise(async (resolve, reject) => {
        try {
            ...
            tbl_userLocal.email, tbl_pricelistjob.jobType, tbl_pricelistjob.price, concat(tbl_employee.firstname, ' ', 
            ...
            ON tbl_customer.userLocalId=tbl_userLocal.id LEFT JOIN tbl_priceListJob ON tbl_orders.priceListId=tbl_priceListJob.id LEFT JOIN tbl_employee 
